...
function parseMarkdownToHTML(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('# ')) {
            ...
            // Question start
            const question = line.replace('**Câu hỏi:**', '').trim();
            html += '<div class="question-item">\n';
            
            if (question.startsWith('`') && question.endsWith('`')) {
                // Code question
                const code = question.slice(1, -1);
            ...
            html += `<div class="answer-highlight">Đáp án: ${answer}</div>\n`;
        } else if (line.startsWith('**Giải thích:**')) {
            // Explanation
            const explanation = line.replace('**Giải thích:**', '').trim();
            html += '<div class="explanation">\n';
            html += `<strong>Giải thích:</strong> ${explanation}\n`;
            html += '</div>\n';
            html += '</div>\n'; // Close question-item
        } else if (line.startsWith('- **')) {
            // List item in conclusion
...
console.log('✅ Generated complete answer keys HTML page with all 219 questions!');
